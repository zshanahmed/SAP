...
class ViewAllyProfileFromAdminDashboard(AccessMixin, View):
    def get(self, request, *args, **kwargs):
        ...
        try:
            ...
            return render(
                request, "sap/admin_ally_table/view_ally.html", {"ally": ally}
            )
        except:
...
class ChangeAdminPassword(AccessMixin, View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        return render(request, "sap/change_password.html", {"form": form})
...
class EditAdminProfile(AccessMixin, View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        return render(request, "sap/profile.html", {"form": form})
    ...
    def post(self, request):
        ...
        if valid:
            ...
            if (
                authenticate(
                    request,
                    username=newAdminDict["current_username"][0],
                    password=newAdminDict["current_password"][0],
                )
                is not None
            ):
                # if are check username exists in database
                if User.objects.filter(
                    username=newAdminDict["new_username"][0]
                ).exists():
                    messages.add_message(
                        request,
                        messages.ERROR,
                        "Account was not created because username exists",
                    )
                    return redirect("/create_iba_admin")
                # Check if repeated password is same
                elif (
                    newAdminDict["new_password"][0]
                    != newAdminDict["repeat_password"][0]
                ):
                    messages.add_message(
                        request,
                        messages.ERROR,
                    ...
                    user = User.objects.create_user(
                        newAdminDict["new_username"][0],
                        newAdminDict["new_email"][0],
                        newAdminDict["new_password"][0],
                    )
                    user.is_staff = True
                    user.save()
...
class SignUpView(TemplateView):
    ...
    def post(self, request):
        ...
        if User.objects.filter(username=postDict["new_username"][0]).exists():
            ...
            user = User.objects.create_user(
                username=postDict["new_username"][0],
            ...
            if postDict["roleSelected"][0] == "Staff":
                ...
                if postDict["roleSelected"][0] == "Undergraduate Student":
                    ...
                    ally = Ally.objects.create(
                        ...
                        willing_to_volunteer_for_events=selections[
                            "volunteerGradRadios"
                        ],
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            if len(valid_email) > 0:
                user = valid_email[0]
                user.is_active = (
                    False  # User needs to be inactive for the reset password duration
                )
                # user.profile.reset_password = True
                user.save()

                message_body = render_to_string(
                    "sap/password-forgot-mail.html",
                    {
                        "user": user,
                        "protocol": "http",
                        "domain": site.domain,
...
class ForgotPasswordConfirmView(TemplateView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if user is not None and password_reset_token.check_token(user, token):
            ...
            messages.error(
                request,
                "Password reset link is invalid. Please request a new password reset.",
            )
            return redirect("sap:home")
