...
class AdminAllyTableFeatureTests(TestCase):
    ...
    def test_view_ally_page_for_admin(self):
        ...
        self.assertContains(response, "Ally Profile", html=True)
    ...
    def test_view_non_ally_page_for_admin(self):
        ...
        self.user.save()
        self.client.login(username=self.username, password=self.password)
        response = self.client.get("/allies/", {"username": "something"})
    ...
    def test_delete_ally(self):
        ...
        response = self.client.get(
            "/delete/", {"username": self.ally_user.username}, follow=True
        )
        self.assertEqual(response.status_code, HTTPStatus.OK)
...
class AdminUpdateProfileAndPasswordTests(TestCase):
    ...
    def test_update_profile_page_for_admin(self):
        ...
        self.assertEqual(response.status_code, HTTPStatus.OK)
        self.assertContains(response, "Update Admin Profile", html=True)
...
class AlliesIndexViewTests(TestCase):
    ...
    def test_one_ally(self):
        ...
        response = self.client.get(reverse("sap:sap-dashboard"))
        self.assertQuerysetEqual(response.context["allies_list"], ["<Ally: hawk_id_1>"])
    ...
    def test_two_allies(self):
        ...
        self.assertQuerysetEqual(
            ...
            [
                "<Ally: hawk_id_2>",
                "<Ally: hawk_id_1>",
            ],  # Need to return in the descending order (When the ally record was created in the table)
        )
    ...
    def test_post_missing_data(self):
        response = self.c.post(
            "/create_iba_admin/",
            {
                ...
                "current_password": ["iba_sep_1"],
                "new_email": [""],
                "new_username": ["iba_admin"],
                "new_password": ["iba_sep_1"],
    ...
    def test_post_Invalid_credentials(self):
        response = self.c.post(
            "/create_iba_admin/",
            {
                ...
                "current_password": ["admin_assword1"],
                "new_email": ["emailGuy@email.com"],
                "new_username": ["iba_admin"],
                "new_password": ["iba_sep_1"],
                "repeat_password": ["iba_sep_1"],
            },
    ...
    def test_post_new_username_that_exists(self):
        response = self.c.post(
            "/create_iba_admin/",
            {
                ...
                "current_username": ["admin"],
                "current_password": ["admin_password1"],
                "new_email": ["emailGuy@email.com"],
                "new_username": ["admin"],
                "new_password": ["admin_password1"],
                "repeat_password": ["iba_sep_1"],
            },
        )
...
class LogoutRedirectTests(TestCase):
    def setUp(self):
        ...
        User.objects.create_user(
            self.username, "email@test.com", self.password, is_staff=True
        )
...
class SignUpTests(TestCase):
    ...
    def test_entered_existing_user(self):
        response = self.c.post(
            ...
            {
                "csrfmiddlewaretoken": [
                    "MIyNUVJILbLGKrHXjz4m4fWt4d13TUOkkvRtCpStSmxkW8PKomuz3ESTYF8VVQil"
                ],
                "firstName": ["Elias"],
                "lastName": ["Shaeffer"],
                "new_username": ["admin"],
                "new_email": ["eshaeffer@uiowa.edu"],
                "new_password": ["dddddddd"],
                "repeat_password": ["dddddddd"],
    ...
    def test_create_Undergrad(self):
        response = self.c.post(
            ...
            {
                "csrfmiddlewaretoken": [
                    "At4HFZNsApVRWNye2Jcj4RVcWYf1fviv1kFbSZevLnNmJrWz4OyZhcAPn0JeaknZ"
                ],
                "firstName": ["Zeeshan"],
                "lastName": ["Ahmed"],
        ...
        categoryRelation = AllyStudentCategoryRelation.objects.filter(
            ally_id=ally[0].id
        )
        categories = StudentCategories.objects.filter(
            id=categoryRelation[0].student_category_id
        )
    ...
    def test_create_Grad(self):
        response = self.c.post(
            ...
            {
                "csrfmiddlewaretoken": [
                    "TFosu1rFWp6S4SsYIV5Rb9FtBzoTavgrCsu31o9hTp975IuRpZeNgPJeBQiU6Cy5"
                ],
                "firstName": ["glumpy"],
                "lastName": ["guy"],
    ...
    def test_Faculty(self):
        response = self.c.post(
            ...
            {
                "csrfmiddlewaretoken": [
                    "gr9bKWMJLFrJZfcdKkdRhlyKLI0JeTh2ZefMhjulIFuY05e6romNm1CvLZUKa0zG"
                ],
                "firstName": ["Terry"],
                "lastName": ["Braun"],
                "new_username": ["tbraun"],
...
class NonAdminAccessTests(TestCase):
    def setUp(self):
        ...
        User.objects.create_user(
            self.username, "email@test.com", self.password, is_staff=False
        )
...
class ForgotPasswordTest(TestCase):
    ...
    def test_get(self):
        ...
        self.assertEqual(response.status_code, HTTPStatus.OK)
        self.assertContains(response, "Send me instructions!", html=True)
    ...
    def test_reset_password_success(self):
        ...
        data = {"new_password1": "user_password2", "new_password2": "user_password2"}
        form = UserResetForgotPasswordForm(user=self.user, data=data)
        self.assertTrue(form.is_valid())
    ...
    def test_reset_password_failure(self):
        ...
        data = {"new_password1": "user_password2", "new_password2": "wrong_password"}
        form = UserResetForgotPasswordForm(user=self.user, data=data)
        self.assertFalse(form.is_valid())
