...
# Scenarios
scenarios('../features/admin_view_ally.feature')
...
@when(parsers.parse('I click on view button correspoding to a row on table'))
def goto_change_password(chromeBrowser):
    ...
    if len(rows) > 0:
        ...
        view_button = all_buttons[0].find_element(By.TAG_NAME, "a").click()

    
@then(parsers.parse('I should see the profile page with information of that Ally'))
def goto_change_password(chromeBrowser):
    url = chromeBrowser.current_url
    if 'allies' in url:
        head_text = chromeBrowser.find_element(By.TAG_NAME, "h1").text
        assert head_text == 'Ally Profile'
